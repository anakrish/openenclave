...
// https://github.com/Tencent/rapidjson/blob/master/doc/tutorial.md
...
int enc_main(int argc, const char** argv)
{
    ...
    assert(
	document
	.IsObject()); // Document is a JSON value represents the root of
    ...
    assert(document["i"]
	   .IsInt()); // In this case, IsUint()/IsInt64()/IsUint64()
    // also return true.
    printf(
	"i = %d\n",
	...
	printf("a = ");
	for (Value::ConstValueIterator itr = a.Begin(); itr != a.End();
	     ++itr)
	    printf("%d ", itr->GetInt());
	printf("\n");
    }
	...
	assert(!document["i"]
	       .IsInt()); // No longer can be cast as int or uint.
    }
	...
	// is needed for potentially realloc.
	// We normally uses the document's.
    ...
    // So it is for literal and string that exists within value's
    // life-cycle.
    {
	...
	author.SetString(
	    buffer2, static_cast<SizeType>(len), document.GetAllocator());
    ...
    // Variable 'buffer' is unusable now but 'author' has already made a
    // copy.
    document.AddMember("author", author, document.GetAllocator());

    assert(author.IsNull()); // Move semantic for assignment. After this
    // variable is assigned as a member, the
